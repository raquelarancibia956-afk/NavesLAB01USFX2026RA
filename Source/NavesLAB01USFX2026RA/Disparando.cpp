...
void ADisparando::Disparar(float forwardValue, float rightValue)
{
	...
	if (Jugador->bCanFire == true)
	{
		...
		if (FireDirection.SizeSquared() > 0.0f)
		{
			...
			const FVector SpawnLocation = GetActorLocation() + FireRotation.RotateVector(Jugador->GunOffset);
...
void ADisparando::Moverse(float forwardValue, float rightValue, float bCorrer, float DetlaTime)
{
